...
use crate::models::{Response, Station, StationPoint};
...
async fn get_stations(Query(params): Query<SqlQuery>) -> AppResult<Json<Response<Station>>> {
    let start_time = Instant::now();
    let query = params
        .query
        .unwrap_or_else(|| "SELECT * FROM self;".to_string());
